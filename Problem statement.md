...
**Level 2: Pattern Recognition** : Assesses visual memory through timed number sequence recall.The user will need to select the same exact number pattern as it is displayed on the screen.One wrong number would result in game over. 
...
By linking all levels through an extensible level architecture and offering administrative cheat code overrides to skip stages, the system provides a continuous, balanced, and dynamic single-player experience. 

When we think about the system we have to consider the side of things. The system needs to be built in a way that meets a lot of requirements. The System must act as a proof of concept for the following things: 

The System must be fully planned out using diagrams that show how all the parts fit together like Use Case diagrams, Class diagrams, Sequence diagrams, Activity diagrams, State Machine diagrams and Package diagrams. These diagrams must show that we understand how to use multiplicity, aggregation, composition and interface realization in the System. The System needs to use Object Oriented Programming principles like Encapsulation to keep health data safe, Abstraction to make abstract Level classes Inheritance to create a hierarchy of Level classes and Polymorphism to make the code more flexible when executing levels in the System. 
