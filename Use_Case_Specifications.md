...
**Stakeholder:**
* **Player-** The player will run the game from the beginning and follow a linear game journey of different levels and challenges.
* **System Developer-**Uses OOP (Encapsulation and Array/List of polymorphic objects) to control campaign progression. 

**e) Preconditions:**
1) The console application should be launched successfully.
2) The initialised memory is the level queue.

**f) Postconditions:**
1) System loads level 1 into memory.
2) The campaign journey begins.

**g) Trigger:** Player opens the application and selects “start journey”.

### 2) MAIN FLOW:
* System displays welcome title screen with player options (Start journey [1], Quit journey [2]).
* Player inputs 1 to start the journey.
* System displays the level information of the first level(The riddle).
* System transfers control to the execution loop of Level 1 (proceeds to UC-02).

### 3. Alternate / Exceptional Flows
A specific keyword that a user can use to skip a level. 
...
### 2. Main flow:
* System renders current stage display in console and prompts for input.
* Player inputs action/move command.
* System validates input against current level's rules and handles the exception cases.
* System updates internal stage state variables(health,stamina,etc).
* System checks winning/completion criteria for the current stage.
* System re-renders updated console view.
* Stage completion criteria met: System flags stage state as complete and ends active game loop (Proceeds to UC-03).

### 3. Alternate / Exceptional Flows

**A1: Invalid Command Input**
1. Player inputs out-of-bounds or misformatted data.
2. System catches error, displays prompt: "Invalid input for this stage. Try again."
...
3. System displays failure screen ("Stage Failed!") and gives option to [1] Retry Level or [2] Quit Journey.
...
* System displays Level Clear victory banner and calculates score/bonus for this stage.
* System updates total player campaign statistics.  (Health, Stamina, etc)
* System checks if additional levels remain in the campaign sequence queue (currentIndex < totalLevels).
* System increments level index (currentIndex++).
* System displays transition screen ("Stage Unlocked! Press ENTER to enter Level [X]").
* Player presses ENTER.
* System loads next Level object polymorphically and transfers control (returns to Step 5 of UC-01).
